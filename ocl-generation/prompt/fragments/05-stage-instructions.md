...
如果 RAG 动态样例区为空（RAG 不可用），阶段 4 必须提高风险提示级别：对使用以下任一特征的规则，置信度至少降一级，并在风险点中标注「无 RAG 相似样例参考」：关联导航、集合操作（如 `->size()`/`->select()`/`->collect()`）、量词（`forAll`/`exists`）、聚合（如 `sum`）、条件表达式（`if-then-else`）。
