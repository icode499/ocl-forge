...
`<stage3>` 的关联构造请按多重性区分：单值关联使用 `set<Assoc>(...)`，集合关联使用 `add<Assoc>(...)`。
...
| 3 | 移除 RAG 动态区，仅依赖固定样例（仅当 Golden Examples 已完成选取并填充后适用） |
| 4 | 固定样例从 5 个缩减到 3 个（保留 #1 属性、#2 集合、#5 复合；仅当 Golden Examples 已完成选取并填充后适用） |

不要缩减 fragments 区（角色定义、语法速查、阶段指令等），这些是生成质量的基础。
当前 `golden-v0` 阶段若固定样例区仍未填充，不应把“仅依赖固定样例”当作可用兜底策略。
